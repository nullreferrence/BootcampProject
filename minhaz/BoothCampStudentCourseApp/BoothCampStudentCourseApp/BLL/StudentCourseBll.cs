...
;        }
        ...
        {
            if (aStudentGateway.GetStudentNameByRegNo(regNo) == "")
            {
        ...
        {
            if (aStudentGateway.GetEmailAddressByRegNo(regNo) == "")
            {
        ...
        public List<StudentCourses> GetOnlyEnrolledCourseComboBoxList(int studentId)
        {
            List<StudentCourses> aCourseNameList = aCourseGateway.GetEnrolledCourseName(studentId);
            return aCourseNameList;
        }

        public string SaveResult(StudentCourses anStudentCourses)
        {
            aCourseGateway.SaveResult(anStudentCourses);
            return "Result Saving Successfull";
        }
    }
}
